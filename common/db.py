...
def dbConnect():
	cnx = mysql.connector.connect(user=c.DBUSER, password=c.DBPWD,host =c.DBHOST,database=c.DB,use_pure=True)
	cursor = cnx.cursor()
...
def get_topic_labels(domain):
	sql = "SELECT topic_id, label FROM TopicKeywords_3gram_"+domain+"_"+str(c.domain_topics[domain])
	return sql
